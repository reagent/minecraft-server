...
|----------|---------|---------|
| `provision.yml` | Manual (`workflow_dispatch`) | Create droplet, attach `minecraft-data` volume, install server. Inputs: `region` (single-option choice, currently `sfo3`), `size`, `duration_hours` (used for auto-destroy), `world` (choice — drives both the DNS hostname `<world>.minecraft.<domain>` and Minecraft's `level-name`, so each world keeps its own subdirectory on the persistent volume; add new worlds by editing the `options` list in `provision.yml`). |
